...
object ScholashipRecipientJoinFilter {
  def main(args: Array[String]): Unit = {
    ...
    spark.sparkContext.hadoopConfiguration.set("fs.s3n.awsAccessKeyId", "AKIAUDH2QU745GQTNAGT")
    spark.sparkContext.hadoopConfiguration.set("fs.s3n.awsSecretAccessKey", "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")

    val demographicsRDD = spark.sparkContext.textFile(s"s3n://dsmdf/demographic.csv")
    val financesRDD = spark.sparkContext.textFile(s"s3n://dsmdf/finances.csv")
    val coursesRDD = spark.sparkContext.textFile(s"s3n://dsmdf/course.csv")
